...
const TIMELINE_STEPS: TimelineStep[] = [
  ...
  },
  {
    ...
    badge: '야생의 바닥',
    badgeColor: '#f04452',
    title: '퇴사 후 마주한 야생의 피: 20만 뷰의 저주와 셧다운',
    subtitle: '강의 팔이들의 "AI 퇴사 월 천만 원" 허상 폭로, 직접 내 돈을 태우며 겪은 실패들',
    quote: '"조회수 20만이 터졌는데 통장에 찍힌 결제는 단 4건이었습니다."',
    description: '인터넷 강의에서 말하는 "AI로 뚝딱 자동 수익"은 완전히 거짓이었습니다. 직접 프론트엔드와 백엔드를 구축하고, 매달 내 생돈으로 메타 광고비와 서버비를 태우며 뼈아픈 실패를 연달아 맞았습니다. 20.7만 조회수 대박 글이 터졌으나 결제는 4건(CVR 0.0019%)뿐이었고, 이어진 알고리즘 숙취로 5일간 유입이 -92% 폭락했습니다. 하루 10개 글을 난사했다가 계정이 48회로 셧다운당하기도 했습니다.',
    metrics: [
    ...
    bulletPoints: [
      '비즈니스 결제 의도 없는 대중 밈 바이럴의 치명적 독성(알고리즘 숙취) 체감',
      '하루 10편 발행 과다로 계정 품질 점수(Quality Score) 강등 및 48회 셧다운',
      '광고 소재와 랜딩페이지 불일치로 인한 광고비 누수(전환율 0%) 참사'
    ],
    icon: Flame
  },
  {
    ...
    badge: '엔지니어링 반전',
    badgeColor: '#10b981',
    title: '감정적 좌절 대신 DB와 코드로 메타 알고리즘을 역공학하다',
    subtitle: 'PostgreSQL 결제 로그 조인, Two-Tower DLRM 분해, 광고 ↔ 랜딩 1:1 회수로 CVR 8.6% 달성',
    quote: '"문제가 생기면 울지 않고 SQL 쿼리를 날리고 코드를 뜯어고칩니다."',
    description: '실패에 좌절하는 대신 엔지니어로서 데이터베이스(Neon DB) 결제 테이블을 조인하고 메타 Graph API 로그를 전수 조사했습니다. 695편의 포스트와 423만 뷰 데이터를 대조해 1,000~3,000뷰의 황금 스위트 스팟(CVR 4.49%)을 밝혀냈고, 광고 장면과 랜딩 첫 카드를 1:1로 일치시키는 "소오름 매칭"으로 메타 광고 결제 전환율 8.6%를 달성했습니다. 월 $42 인프라로 4개 상용 서비스를 혼자 굴리는 파이프라인을 완성했습니다.',
    metrics: [
      { label: '전수 분석 데이터', value: '423만 뷰 · 695편' },
      { label: '황금 스위트스팟', value: 'CVR 4.49% (1~3천뷰)' },
      { label: '메타 광고 CVR', value: '8.6% (증거물 앵글)' }
    ],
    bulletPoints: [
      '광고 장면 ↔ 랜딩 첫 카드 1:1 회수 (소오름 매칭)로 결제 전환율 8.6% 극대화',
      'First 9 텍스트 임베딩(Sentence-BERT) 순도로 3초 프로필 전환 공식 정립',
      'V0 + Cursor + Supabase + Cloudflare로 월 $42 풀스택 인프라 구축'
    ],
    icon: TerminalWindow
  },
  {
